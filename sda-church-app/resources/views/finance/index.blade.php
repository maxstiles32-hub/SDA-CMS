        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 print:block">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div x-data="{ activeTab: 'offering' }">
                            <div class="flex gap-2 mb-4">
                                <button type="button" @click="activeTab = 'offering'"
                                    :class="activeTab === 'offering' ? 'bg-teal-600 text-white' : 'bg-gray-100 text-gray-600'"
                                    class="px-3 py-1 text-xs font-bold rounded-full transition">Offering</button>
                                <button type="button" @click="activeTab = 'donation'"
                                    :class="activeTab === 'donation' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'"
                                    class="px-3 py-1 text-xs font-bold rounded-full transition">Donation</button>
                            </div>
                            ...
                            <div x-show="activeTab === 'donation'">
                                <form method="POST" action="{{ route('finance.donation.store') }}" class="space-y-4"
                                    ...
                                    <button type="submit"
                                        class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 transition">Save
                                        Donation</button>
                                </form>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-2 space-y-6 print:mt-8">
                    ...
                    <div
                        class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 print:p-0 print:border-none print:shadow-none break-inside-avoid shadow-lg transform transition hover:shadow-xl">
                        <div class="flex justify-between items-center mb-4 border-b pb-2">
                            ...
                            <span
                                class="text-xs bg-red-100 text-red-600 px-2 py-1 rounded font-bold uppercase">Outflow</span>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead>
                                    <tr>
                                        <th class="px-3 py-2 text-left text-xs font-bold text-gray-500 uppercase">Date
                                        </th>
                                        <th class="px-3 py-2 text-left text-xs font-bold text-gray-500 uppercase">Title
                                        </th>
                                        <th class="px-3 py-2 text-left text-xs font-bold text-gray-500 uppercase">
                                            Category</th>
                                        <th class="px-3 py-2 text-right text-xs font-bold text-gray-500 uppercase">
                                            Amount</th>
                                        <th class="px-3 py-2 text-left text-xs font-bold text-gray-500 uppercase">Method
                                        </th>
                                        <th
                                            class="px-3 py-2 text-right text-xs font-bold text-gray-500 uppercase print:hidden">
                                            Actions</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    @forelse($expenditures as $exp)
                                        <tr
                                            class="hover:bg-red-50 transition border-l-4 border-transparent hover:border-red-500">
                                            ...
                                            <td class="px-3 py-2 text-sm text-red-600 font-black text-right">(GHS
                                                {{ number_format($exp->amount, 2) }})
                                            </td>
                                            <td class="px-3 py-2 text-sm text-gray-500 uppercase tracking-tighter">
                                                {{ $exp->payment_method }}
                                            </td>
                                            <td class="px-3 py-2 text-right text-sm font-medium print:hidden">
                                                <form
                                                    action="{{ route('finance.expenditure.destroy', $exp->expenditure_id) }}"
                                                    method="POST" onsubmit="return confirm('Delete this expenditure?');"
                                                    class="inline">
                                                    @csrf @method('DELETE')
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="px-3 py-4 text-center text-sm text-gray-500 italic">No
                                                expenditures yet.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                    ...
                    <div
                        ...
                        <h3
                            class="text-lg font-bold text-gray-800 border-b-2 border-emerald-200 pb-2 mb-4 flex items-center">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($tithes as $tithe)
                                        <tr class="hover:bg-emerald-50">
                                            <td class="px-3 py-2 text-sm text-gray-500">
                                                {{ $tithe->date_received->format('M d') }}
                                            </td>
                                            <td class="px-3 py-2 text-sm text-gray-900">{{ $tithe->member->first_name }}
                                                {{ $tithe->member->last_name }}
                                            </td>
                                            <td class="px-3 py-2 text-sm text-emerald-600 font-bold text-right">GHS
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            @foreach($offerings->take(5) as $off)
                                <div class="flex justify-between text-xs py-1">
                                    <span class="text-gray-500">{{ $off->date_received->format('M d') }} -
                                        {{ $off->category }}</span>
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            @foreach($donations->take(5) as $don)
                                <div class="flex justify-between text-xs py-1">
                                    <span class="text-gray-500">{{ $don->date_received->format('M d') }} -
                                        {{ Str::limit($don->purpose, 15) }}</span>
